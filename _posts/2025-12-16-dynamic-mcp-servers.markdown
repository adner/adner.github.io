...
- The `ea9ffc3e-8a23-4a7d-836d-234d7c7565c1/.default` scope is a blanket request to get all scopes that we previously defined for all App Registration (we only defined one scope, the `McpServers.DataverseCustom.All` scope, which is the one that is needed here, but `.default` is a shorthand for getting all of them). 
