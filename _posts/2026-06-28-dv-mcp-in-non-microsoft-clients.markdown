...
- I also blogged about using Dataverse MCP Server from [Semantic Kernel](https://nullpointer.se/2025/07/19/semantic-kernel-mcp.html), the agent orchestration framework available from Microsoft at that time. Since then, Microsoft has released [Microsoft Agent Framework](https://learn.microsoft.com/en-us/agent-framework/overview/), a framework wastly superior to Semantic Kernel.
